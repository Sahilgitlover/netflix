...
const AuthForm: React.FC<stateOfProp> = ({ isSignUp }) => {
  ...
  const submitHandler = async (data: FormData) => {
    ...
    try {
      ...
      router.replace(
        isSignUp ? `/verify/${response.data.id}` : `/SelectProfile`
      );
    } catch (error) {
  ...
  return (
    <div id="forBg">
      <div className="content">
        <Navbar signUp={isSignUp} setLoading={setLoading} />
      </div>
      {!loading ? (
        <div>
              ...
              <div className="relative z-[2] m-12">
                <form onSubmit={handleSubmit(submitHandler)}>
                  ...
                  <div className="input-container relative">
                    ...
                    {passVisible ? (
                      <FaRegEyeSlash
                        className="absolute top-[14px] right-3 text-xl pointer"
                  ...
                  {!isSignUp && (
                    <div className="text-center mt-4">
                      <a
                        href="#"
                        className="text-white hover:text-slate-300 hover:underline"
                      >
                        Forgot password?
                      </a>
                    </div>
